...
void main() {
  ...
  test('buildReceiptFromPng converts relay PNG to ESC/POS bytes', () async {
    const png = <int>[
    ...
    final bytes = await EscPosReceiptBuilder.buildReceiptFromPng(
      Uint8List.fromList(png),
      printerName: 'Xprinter XP-80C',
    );
    expect(bytes, isNotEmpty);
    expect(bytes.first, 27); // ESC reset
  });
}
